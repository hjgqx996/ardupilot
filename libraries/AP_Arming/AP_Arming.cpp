...
bool AP_Arming::mission_checks(bool report)
{
    if (((checks_to_perform & ARMING_CHECK_ALL) || (checks_to_perform & ARMING_CHECK_MISSION)) &&
        ...
        for (int mask = 1; mask < MIS_ITEM_CHECK_MAX; mask <<= 1) {
            switch (_required_mission_items & mask) {
                case MIS_ITEM_CHECK_LAND:
                    if (!mission->contains_item(MAV_CMD_NAV_LAND)) {
                        check_failed(ARMING_CHECK_MISSION, report, "Missing mission item: land");
                        return false;
                    }
                    break;
                case MIS_ITEM_CHECK_VTOL_LAND:
                    if (!mission->contains_item(MAV_CMD_NAV_VTOL_LAND)) {
                        check_failed(ARMING_CHECK_MISSION, report, "Missing mission item: vtol land");
                        return false;
                    }
                    break;
                case MIS_ITEM_CHECK_DO_LAND_START:
                    if (!mission->contains_item(MAV_CMD_DO_LAND_START)) {
                        check_failed(ARMING_CHECK_MISSION, report, "Missing mission item: do land start");
                        return false;
                    }
                    break;
                case MIS_ITEM_CHECK_TAKEOFF:
                    if (!mission->contains_item(MAV_CMD_NAV_TAKEOFF)) {
                        check_failed(ARMING_CHECK_MISSION, report, "Missing mission item: takeoff");
                        return false;
                    }
                    break;
                case MIS_ITEM_CHECK_VTOL_TAKEOFF:
                    if (!mission->contains_item(MAV_CMD_NAV_VTOL_TAKEOFF)) {
                        check_failed(ARMING_CHECK_MISSION, report, "Missing mission item: vtol takeoff");
                        return false;
                    }
                    break;
                case MIS_ITEM_CHECK_RALLY:
                    {
                        Location ahrs_loc;
                        if (!AP::ahrs().get_position(ahrs_loc)) {
